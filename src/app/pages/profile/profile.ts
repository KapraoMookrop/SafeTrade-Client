...
export class Profile {
  constructor(public stateService: AppStateService,
    ...
    private readonly AutgService: AuthService) {
  }
  ...
  async VerifyEnable2FA() {
    try {
      ...
      this.AutgService.UpdateUser({ IsEnabled2FA: true });
      ...
      this.AutgService.UpdateUser({ IsEnabled2FA: this.stateService.user()?.IsEnabled2FA });
      this.loadingService.hide();
    }
  }
  ...
  async Disable2FA() {
    try {
      const confirm = await Swal.fire({
        ...
        text: 'คุณแน่ใจหรือไม่ว่าต้องการปิดใช้งาน 2FA?',
        icon: 'warning',
        showCancelButton: true,
      ...
      if (confirm.isConfirmed) {
        ...
        Swal.fire({
          icon: 'success',
          title: 'ยืนยัน 2FA สำเร็จ',
          text: 'คุณได้ปิดใช้งาน 2FA แล้ว',
        });
        this.AutgService.UpdateUser({ IsEnabled2FA: false });
      }
    } catch (err: HttpErrorResponse | any) {
      Swal.fire({
        icon: 'error',
  ...
  async Toggle2FA(event: Event) {
    ...
    if (this.stateService.user()?.IsEnabled2FA) {
  ...
  LogOut() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.AutgService.ClearUserClient();
